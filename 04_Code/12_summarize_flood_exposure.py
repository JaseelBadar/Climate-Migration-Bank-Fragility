...
log_lines.append("\n[2] EXPOSURE RATES")
log_lines.append(
    f"   Rule A (full sample):      {df['flood_exposure_ruleA_qt'].sum():,} "
    f"district-quarters ({ruleA_pct:.2f}%)"
)
log_lines.append(
    f"   Rule B (high-precision):   {df['flood_exposure_ruleB_qt'].sum():,} "
    f"district-quarters ({ruleB_pct:.2f}%)"
)
...
log_lines.append("\n[4] TOP 15 EXPOSED DISTRICTS (Rule A)")
top_districts_A = (
    df[df['flood_exposure_ruleA_qt'] == 1]
    .groupby(['district_gadm', 'state_gadm']).size()
...
log_lines.append("\n[5] TOP 15 EXPOSED DISTRICTS (Rule B)")
top_districts_B = (
    df[df['flood_exposure_ruleB_qt'] == 1]
    .groupby(['district_gadm', 'state_gadm']).size()
    .sort_values(ascending=False).head(15)
)
...
log_lines.append(
    "   - State-level fallback (Rule A) may introduce measurement error"
)
log_lines.append(
    "   - Historical district names (e.g., Cuddapah -> Kadapa) "
    "not fully resolved"
)
log_lines.append(
    "   - Rule B exposure rate (1.02%) reflects EM-DAT district precision limit"
)
...
log_lines.append(
    "   PASS - Event 2019-0331-IND: 33 Rule B districts correctly coded"
)
log_lines.append(
    "   PASS - Event 2023-0428-IND: State fallback triggered for HP, Delhi "
    "(Rule A only). Rule B = 9 reflects other 2023Q2 events, not this event."
)
log_lines.append(
    "   PASS - Event 2015-0504-IND: 9/10 AP/TN districts matched (90% rate)"
)
...
output_path = '05_Outputs/Logs/12_flood_exposure_summary.txt'
