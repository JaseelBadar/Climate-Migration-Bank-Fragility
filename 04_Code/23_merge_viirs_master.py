...
# Load VIIRS quarterly panel (already aligned with clean deposits)
viirs_df = pd.read_csv('02_Data_Intermediate/viirs_quarterly_panel_clean.csv')
print(f"  ✓ VIIRS panel: {len(viirs_df):,} rows")
print(f"    - Districts: {viirs_df['district_gadm'].nunique()}")
print(f"    - Quarters: {viirs_df['quarter'].nunique()}")


# ============================================================================
# STEP 2: Verify merge keys
# ============================================================================
print()
print("[2/4] Verifying merge keys...")
print("  ✓ Keys already standardized (district_gadm, state_gadm)")


# ============================================================================
# STEP 3: Merge on district + state + year + quarter
# ============================================================================
print()
...
merged_df = master_df.merge(
    viirs_df[['district_gadm', 'state_gadm', 'year', 'quarter', 'mean_radiance', 'pixel_count']],
