...
Validate viirs_quarterly_panel_clean.csv before regression pipeline.
Checks: dimensions, columns, district coverage, temporal coverage,
NaN/Inf, radiance range, balance, homonymous districts.
CORRECTED 2026-03-06: expected_rows/districts fixed 624->631,
Aurangabad litmus test case normalised to UPPERCASE.
"""
import pandas as pd
import numpy as np
import logging
...
expected_rows = 631 * 40  # 25,240
actual_rows   = len(df)

print(f"  Expected: {expected_rows:,} rows (631 districts x 40 quarters)")
print(f"  Actual:   {actual_rows:,} rows")
log.info(f"Expected: {expected_rows:,} rows (631 districts x 40 quarters)")
...
if actual_rows == expected_rows:
    ...
    print(f"  FAIL: Difference of {abs(expected_rows - actual_rows):,} rows")
...
required_cols = ['district_gadm', 'state_gadm', 'year', 'quarter',
...
expected_districts         = 631

print(f"  Expected: {expected_districts} districts (composite count)")
print(f"  Actual (name only):  {unique_districts_name} "
      f"(should be 624 if 7 homonymous pairs exist)")
...
if not missing_quarters:
    ...
    print(f"  FAIL: Missing {len(missing_quarters)} quarters: {missing_quarters[:5]}")
    log.warning(f"RESULT: FAIL - Missing quarters: {missing_quarters}")
    validation_passed = False

# === CHECK 5: MISSING VALUES (NaN) ===
...
print(f"  mean_radiance NaN: {nan_counts['mean_radiance']:,} "
      f"({nan_counts['mean_radiance']/len(df)*100:.2f}%)")
print(f"  pixel_count NaN:   {nan_counts['pixel_count']:,} "
      f"({nan_counts['pixel_count']/len(df)*100:.2f}%)")
log.info(f"mean_radiance NaN: {nan_counts['mean_radiance']:,} "
         f"({nan_counts['mean_radiance']/len(df)*100:.2f}%)")
log.info(f"pixel_count NaN:   {nan_counts['pixel_count']:,} "
         f"({nan_counts['pixel_count']/len(df)*100:.2f}%)")
...
if nan_counts['mean_radiance'] == 0:
    ...
    print(f"  FAIL: {nan_counts['mean_radiance']} NaN values in mean_radiance")
    log.warning(f"RESULT: FAIL - {nan_counts['mean_radiance']} NaN values detected")
    validation_passed = False

# === CHECK 6: INFINITE VALUES (Inf) ===
...
if negative_count > 0:
    print(f"  FAIL: {negative_count} negative radiance values")
...
log.info(f"Actual: Min={obs_per_district.min()}, "
         f"Max={obs_per_district.max()}, "
         f"Mean={obs_per_district.mean():.1f}")
...
if obs_per_district.min() == obs_per_district.max() == 40:
    ...
    imbalanced = obs_per_district[obs_per_district != 40]
    print(f"  FAIL: {len(imbalanced)} districts with != 40 observations")
    log.warning(f"RESULT: FAIL - {len(imbalanced)} districts unbalanced")
...
if len(homonymous_districts) > 0:
    ...
    # Aurangabad litmus test — UPPERCASE to match Script 22b normalisation
    if 'AURANGABAD' in homonymous_districts.index:
        print(f"\n  Aurangabad litmus test (UPPERCASE keys):")
        aug_bihar = df[
            (df['district_gadm'] == 'AURANGABAD') &
            (df['state_gadm']    == 'BIHAR')
        ]['mean_radiance'].mean()
        aug_maha  = df[
            (df['district_gadm'] == 'AURANGABAD') &
            (df['state_gadm']    == 'MAHARASHTRA')
        ]['mean_radiance'].mean()
        ...
        if abs(aug_bihar - aug_maha) > 0.01:
            ...
            print(f"    FAIL: Similar radiance values (possible contamination)")
