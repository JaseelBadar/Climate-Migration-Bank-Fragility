...
# CRITICAL: Use composite district_state_id to prevent homonymous district collapse
# 7 homonymous pairs exist: Aurangabad, Balrampur, Bijapur, Bilaspur, Hamirpur, Pratapgarh, Raigarh
# Without state, 624 districts → 617 FE (wrong); with state → 624 FE (correct)

# Create composite district-state ID
...
log.info("FE: District + Quarter (district_state_id composite keys)")
...
try:
    model = ols(formula, data=df_reg).fit(cov_type='cluster', cov_kwds={'groups': df_reg['district_state_id']})
