...
# Calculate clustered SEs for first stage
...
# Calculate clustered SEs for second stage
...
coef = beta_second[0]
se = se_second_clustered[0]  # <-- NOW USES CLUSTERED SE
t_stat = coef / se
...
with open('05_Outputs/Logs/28_H2_regression.txt', 'w', encoding='utf-8') as f:
    ...
    f.write(f"  β̂  = {coef_first:.6f}\n")
    f.write(f"  SE = {se_first:.6f} (clustered)\n")
    f.write(f"  t  = {t_first:.3f}\n")
    f.write(f"  p  = {p_first:.6f}\n")
    f.write(f"  N  = {len(y_first):,}\n\n")
    
    f.write("[SECOND STAGE: Lights_hat -> Deposits]\n")
    f.write(f"  β̂  = {coef:.6f}\n")
    f.write(f"  SE = {se:.6f} (clustered)\n")
...
print()
print("✓ Script 28 complete (with district-clustered SEs)")