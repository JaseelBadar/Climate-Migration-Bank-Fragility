...
# Sort by district AND state, then quarter to ensure proper ordering
df = df.sort_values(['district_gadm', 'state_gadm', 'quarter']).reset_index(drop=True)


# Create composite district-state ID BEFORE creating lags
df['district_state_id'] = df['district_gadm'] + '_' + df['state_gadm']


# Create lags within each composite district (prevents homonym contamination)
df['flood_t1_lag'] = df.groupby('district_state_id')['flood_exposure_ruleA_qt'].shift(1)
df['flood_t2_lag'] = df.groupby('district_state_id')['flood_exposure_ruleA_qt'].shift(2)
...
print(f"  Lags computed within composite district_state_id")
print()
...
# Calculate clustered standard errors (using composite district_state_id)
residuals = y - (X @ beta)
...
with open('05_Outputs/Logs/29_H3_timing.txt', 'w', encoding='utf-8') as f:
    ...
    f.write(f"  Beta = {coef_t0:.6f}\n")
    f.write(f"  SE   = {se_t0:.6f} (clustered by district_state_id)\n")
    f.write(f"  t    = {t_t0:.3f}\n")
    f.write(f"  p    = {p_t0:.6f}\n")
    f.write(f"  Sig: {sig_t0 if sig_t0 else 'NOT SIGNIFICANT'}\n\n")
    
    f.write("[t-1] 1 Quarter Lag:\n")
    f.write(f"  Beta = {coef_t1:.6f}\n")
    f.write(f"  SE   = {se_t1:.6f} (clustered by district_state_id)\n")
    f.write(f"  t    = {t_t1:.3f}\n")
    f.write(f"  p    = {p_t1:.6f}\n")
    f.write(f"  Sig: {sig_t1 if sig_t1 else 'NOT SIGNIFICANT'}\n\n")
    
    f.write("[t-2] 2 Quarter Lag:\n")
    f.write(f"  Beta = {coef_t2:.6f}\n")
    f.write(f"  SE   = {se_t2:.6f} (clustered by district_state_id)\n")
    f.write(f"  t    = {t_t2:.3f}\n")
    f.write(f"  p    = {p_t2:.6f}\n")
    f.write(f"  Sig: {sig_t2 if sig_t2 else 'NOT SIGNIFICANT'}\n\n")
    
    f.write(f"N = {len(y):,}\n")
    f.write(f"Districts: {len(np.unique(district_ids))} clusters (composite district_state_id)\n\n")
...
print()
print("Script 29 complete (with composite district FE and clustering)")