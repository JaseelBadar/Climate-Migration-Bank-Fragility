...
Purpose:
    ...
    by state_gadm (28 clusters) instead of district_state_id (631 clusters).

Rationale:
    Cameron and Miller (2015): below 50 clusters, standard clustered SEs
    may be liberal (under-reject H0). 28 state clusters is below threshold.
...
Two regressions:
    H1: log_lights_qt ~ flood_exposure_ruleA_qt + C(district_state_id) + C(quarter)
...
log("H1 and H3 re-run with SE clustered by state_gadm (28 clusters)")
log(f"Run: {run_ts}")
log("=" * 70)
...
required_cols = [
    ...
    "deposit_change_qt", "log_lights_qt",
...
log(f"  state_gadm:        {n_states} (expected 28)")

assert n_districts == 631, f"Districts = {n_districts}, expected 631"
assert n_states == 28, f"States = {n_states}, expected 28"
...
def log_coef(label, coef, se, t, p, ci_lo, ci_hi, sig, anchor=None):
    log(f"    [{label}]")
    log(f"      Beta   = {coef:.6f}{' (main: ' + str(anchor['beta']) if anchor else ''}")
    log(f"      SE     = {se:.6f}")
    log(f"      t      = {t:.3f}")
    log(f"      p      = {p:.6f}")
...
formula_h1_a = "log_lights_qt ~ flood_exposure_ruleA_qt + C(district_state_id) + C(quarter)"
...
formula_h1_b = "log_lights_qt ~ flood_exposure_ruleB_qt + C(district_state_id) + C(quarter)"
...
if h1_a_sig and h3_a_sig:
    ...
    log("  28 state clusters. Writing unblocked.")
else:
...
log(f"\n  H1 table saved: {OUT_H1} ({len(h1_df)} rows)")
log(f"  H3 table saved: {OUT_H3} ({len(h3_df)} rows)")
